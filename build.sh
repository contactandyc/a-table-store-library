...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/liba_table_store_library_shared.so"
    NEW_SO="build/liba_table_store_library_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/liba_table_store_library_debug.a" \
