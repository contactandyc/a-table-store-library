...
int pool_wal_init(pool_wal_t* wal, const char* dir, uint64_t segment_size_mb, uint32_t max_standby) {
    ...
    if (!found_segments) {
        ...
        pool_file_header_t hdr = { WAL_MAGIC_NUMBER, 1, 0, 0 };
...
// -----------------------------------------------------------------------------
// STANDBY POOL ROTATION
// -----------------------------------------------------------------------------

static void pool_wal_rotate(pool_wal_t* wal, uint64_t next_seq) {
    close(wal->active_fd);
    ...
    pool_file_header_t hdr = { WAL_MAGIC_NUMBER, wal->current_seg_id, next_seq, 0 };
    pwrite(wal->active_fd, &hdr, sizeof(hdr), 0);
...
int pool_wal_append(pool_wal_t* wal, uint64_t seq, uint8_t type, const uint8_t* payload, uint32_t len) {
    ...
    pwrite(wal->active_fd, &frame_hdr, sizeof(frame_hdr), wal->file_offset);
    if (len > 0) pwrite(wal->active_fd, payload, len, wal->file_offset + sizeof(frame_hdr));
    ...
    while (scan_seg < wal->current_seg_id) {
        ...
        if (next_fd >= 0) {
            if (pread(next_fd, &next_hdr, sizeof(next_hdr), 0) == sizeof(next_hdr)) {
                if (next_hdr.start_seq <= safe_checkpoint) {
                    can_purge = true;
                }
            }
            close(next_fd);
...
static bool lsm_api_append(lsm_wal_t *lsm, uint32_t table_id, uint64_t seq_num, uint8_t op, const void *key, uint32_t klen, const void *val, uint32_t vlen) {
    ...
    pool_wal_append(wal, seq_num, op, buf, payload_len);
    aml_free(buf);
    return true;
}
...
static void lsm_api_checkpoint(lsm_wal_t *lsm, uint32_t table_id, uint64_t seq_num) {
    pool_wal_t *wal = (pool_wal_t *)lsm->ctx;
    ...
    if (!found) {
        ...
        wal->ckpts[wal->num_ckpts].safe_seq = seq_num;
        wal->num_ckpts++;
    }

    uint64_t min_seq = UINT64_MAX;
    for (size_t i = 0; i < wal->num_ckpts; i++) {
        if (wal->ckpts[i].safe_seq < min_seq) {
            min_seq = wal->ckpts[i].safe_seq;
        }
    }
    pthread_mutex_unlock(&wal->mu);

    if (min_seq != UINT64_MAX) {
        pool_wal_purge(wal, min_seq);
    }
}
...
static bool lsm_api_iter_next(lsm_wal_iter_t *iter, uint32_t *out_table_id, uint8_t *out_op,
    ...
    if (!pool_wal_iter_next(w_iter->pool_iter, &seq, &op, &payload, &len)) {
        return false;
    }
