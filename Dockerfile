...
# --- Build & install a-bloom-filter-library ---
RUN set -eux; \
    git clone --depth 1 --single-branch "https://github.com/contactandyc/a-bloom-filter-library.git" "a-bloom-filter-library"; \
    cd "a-bloom-filter-library"; \
    ./build.sh clean && \
    ./build.sh install; \
    cd ..; \
    rm -rf "a-bloom-filter-library"
...
# --- Build & install sql-parser-library ---
RUN set -eux; \
    git clone --depth 1 --single-branch "https://github.com/contactandyc/sql-parser-library.git" "sql-parser-library"; \
    cd "sql-parser-library"; \
    ./build.sh clean && \
    ./build.sh install; \
    cd ..; \
    rm -rf "sql-parser-library"

# --- Build & install this project --------------------------------------------
COPY --chown=dev:dev . /workspace/a-table-store-library
RUN mkdir -p /workspace/build/a-table-store-library && \
    cd /workspace/build/a-table-store-library && \
    cmake /workspace/a-table-store-library && \
